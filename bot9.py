...
def update_prob(type:str,curr_cell:tuple[int,int]):
    ...
    # Bot 9, the probability of hearing the beep (not not hearing the beep) must be updated taking the case that two leaks 
    """
...
def pickMeHighestProbability():
    ...
    global bot_location
    """
    returns the coordinate (binary tuple) of the cell that is in the highest beep section, that has the highest probability. if there is multiple cells that has the same highest probability, then pick the one with the shortest distance from the bot by calling len(BFS_advanced(start_location,target_location)), if multiple cells have same highest probability and same distance away from bot, then randomly choose between them
    """
    # Determine the highest probability section or if it's zero
    highest_section = ship_sections.index(max(ship_sections)) + 1 if max(ship_sections) > 0 else None
    max_prob = -1
    highest_prob_cells = []
    
    for row in range(len(leak_probability)):
        for col in range(len(leak_probability[0])):
            section = determine_section((row, col))
            # Check if the cell is in the highest probability section
            # If highest_section is None, check the entire ship
            if (highest_section is None or section == highest_section) and leak_probability[row][col] > max_prob and ship[row][col] == 1:
...
def determine_section(coordinate):
    """
    Given the tuple coordinate and the size of the ship (width x height),
    return '1' (top left), '2' (top right), '3' (bottom left), or '4' (bottom right),
    each representing a section. Handle uneven splits by assigning extra rows/columns to the bottom/right sections.
    """
    x, y = coordinate
    # Calculate the midpoint, extra cells go to bottom/right sections
    midpoint_x = n // 2
    midpoint_y = n // 2

    if x < midpoint_x:
        return 1 if y < midpoint_y else 2  # Top half
    else:
        return 3 if y < midpoint_y else 4  # Bottom half
...
leak_location = [leak_location1,leak_location2] # BOT9 : two leaks
if debug:
    printShip(ship)
...
# STAGE 4: Bot moves
"""
- The ship is made up of 4 sections
...
total_leak = 2 # BOT9 : multiple leaks
num_actions = 0
try:
    ...
    while True:
        ...
        if debug and GUI_mode:
            ...
            for i in range(ROWS):
                for j in range(COLS):
                    ...
                    elif (i,j) in leak_location and (i,j) == bot_location: # BOT9 : multiple leaks
                    ...
                    elif (i, j) in leak_location: # BOT9 : multiple leaks
                        # leak
        ...
        if clicked or not GUI_mode:
            ...
            if debug and not GUI_mode:
                printShip(ship)
            if bot_location in leak_location: # BOT9 : multiple leaks
                # Leak Found: break the loop
                total_leak -=1 # BOT9 : multiple leaks
            ...
            if(sense_beep(len(BFS_advanced(bot_location,nearestLeak)))):
                ...
                # increase the beep times of the section the bot is at
                ship_sections[determine_section(bot_location)-1]+=1
                printProbList()
            else:
                # Beep is not sensed
                if debug:

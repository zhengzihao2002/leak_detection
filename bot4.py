    ...
    if type == "BEEP":
        ...
        for row in range(len(leak_probability)):
            for col in range(len(leak_probability[row])):
                if (row,col) != curr_cell and ship[row][col]==1:
                    d = len(BFS_advanced(bot_location,leak_location))
        ...
        for row in range(len(leak_probability)):
            for col in range(len(leak_probability[row])):
                if (row,col) != curr_cell and ship[row][col]==1:
                    d = len(BFS_advanced(bot_location,leak_location))
                    leak_probability[row][col] = (leak_probability[row][col] * math.exp(-alpha*(d-1))) / sum
    if type == "NOBEEP":
        # Denominator
        sum = 0
        for row in range(len(leak_probability)):
        ...
        for row in range(len(leak_probability)):
            for col in range(len(leak_probability[row])):
                if (row,col) != curr_cell and ship[row][col]==1:
                    d = len(BFS_advanced(bot_location,leak_location))
                    leak_probability[row][col] = (leak_probability[row][col] * (1-math.exp(-alpha*(d-1)))) / sum
    elif type == "MOVE":
...
bot_location,leak_location=putBotAndLeak(ship)
if debug:
    printShip(ship)
        ...
        if clicked or not GUI_mode:
            ...
            if BFS_in_progress:
                ...
                if debug and GUI_mode:
                    ...
                    input("按回车键继续 ...")
                continue
            if(sense_beep(len(BFS_advanced(bot_location,leak_location)))):
                # Beep is sensed
                if debug:
                    print("BEEP")

...
leak_location = [leak_location1,leak_location2] # BOT5 : two leaks
if debug:
    printShip(ship)
...
total_leak = 2 # BOT5 : multiple leaks
num_actions=0
try:
    if auto_run:
    ...
    while True:
        ...
        if debug and GUI_mode:
            ...
            for i in range(ROWS):
                for j in range(COLS):
                    ...
                    elif (i,j) in leak_location and (i,j) == bot_location: # BOT5 : multiple leaks
                    ...
                    elif (i, j) in leak_location: # BOT5 : multiple leaks
                        # leak
        ...
        if clicked or not GUI_mode:
            ...
            if debug and not GUI_mode:
                ...
                print("possible leak",possible_leak)
                print("no leak",no_leak)
            if bot_location in leak_location: # BOT5 : multiple leaks
                # Leak Found: break the loop
                total_leak -=1 # BOT5 : multiple leaks
